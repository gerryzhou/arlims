...
export class AppInternalUrlsService {
   ...
   {
      return ['test-types', lowerCaseDashSeparated(testTypeCode.toString()), 'test-data-analyzedEntry', testId];
   }

   testStageDataEntry(testTypeCode: LabTestTypeCode, testId: number, stageName: string)
   {
      return ['test-types', lowerCaseDashSeparated(testTypeCode.toString()), 'test-data-analyzedEntry', testId, 'stage', stageName];
   }
